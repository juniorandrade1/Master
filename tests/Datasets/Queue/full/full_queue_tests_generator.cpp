...
namespace Tests {
  ...
  class FullQueueTest {
  public:
    struct FullQueueTestOperation {
      char retroactiveOperation;
      char queueOperation;
      int t;
      int x;
      FullQueueTestOperation(){};
      FullQueueTestOperation(char _retroactiveOperation, char _queueOperation, int _t, int _x) {
    ...
    int q;
    std::vector< FullQueueTestOperation > allOperations;
    FullQueueTest(int _q) {
      q = _q;
      ...
      for(int i = 0; i < q / 2; ++i) {
        ...
        allOperations.push_back(FullQueueTestOperation('I', 'E', t, x));
        used[t] = 1;
        ins.insert(t);
        tr.update(t, N, 1);
      }
      for(int i = 0; i < q - q / 2; ++i) {
        ...
        if(op == 0) {
          ...
          used[t] = 1;
          del.insert(t);
          allOperations.push_back(FullQueueTestOperation('I', 'D', t, INT_MIN));
          tr.update(t, N, -1);
        }
        else if(op == 2) {
          int x = GetSample(ins);
          ins.erase(ins.find(x));
          allOperations.push_back(FullQueueTestOperation('D', 'E', x, INT_MIN));
        }
        else if(op == 3) {
          int x = GetSample(del);
          del.erase(del.find(x));
          allOperations.push_back(FullQueueTestOperation('D', 'D', x, INT_MIN));
        }
        else {
          int t;
          while(1) {
          ...
          allOperations.push_back(FullQueueTestOperation('Q', 'F', t, INT_MIN));
        }
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  for(int i = start; i <= end; i += passo) {
    std::string str = "FullQueue" + std::to_string(i) + ".in";
    FILE *f = fopen(str.c_str(), "w");
    Tests::FullQueueTest v(i);
