...
namespace Tests {
  ...
  class PartialQueueTest {
  public:
    struct PartialQueueTestOperation {
      char retroactiveOperation;
      char queueOperation;
      int t;
      int x;
      PartialQueueTestOperation(){};
      PartialQueueTestOperation(char _retroactiveOperation, char _queueOperation, int _t, int _x) {
        retroactiveOperation = _retroactiveOperation;
        queueOperation = _queueOperation;
        t = _t;
        x = _x;
      }
    };
    int q;
    std::vector< PartialQueueTestOperation > allOperations;
    PartialQueueTest(int _q) {
      q = _q;
      ...
      for(int i = 0; i < q / 2; ++i) {
        ...
        allOperations.push_back(PartialQueueTestOperation('I', 'E', t, x));
        used[t] = 1;
        ins.insert(t);
        tr.update(t, N, 1);
      }
      for(int i = 0; i < q - q / 2; ++i) {
        ...
        if(op == 0) {
          ...
          used[t] = 1;
          del.insert(t);
          allOperations.push_back(PartialQueueTestOperation('I', 'D', t, INT_MIN));
          tr.update(t, N, -1);
        }
        else if(op == 2) {
          int x = GetSample(ins);
          ins.erase(ins.find(x));
          allOperations.push_back(PartialQueueTestOperation('D', 'E', x, INT_MIN));
        }
        else if(op == 3) {
          int x = GetSample(del);
          del.erase(del.find(x));
          allOperations.push_back(PartialQueueTestOperation('D', 'D', x, INT_MIN));
        }
        else {
          allOperations.push_back(PartialQueueTestOperation('Q', 'F', INT_MIN, INT_MIN));
        }
      }
    }
    void showAllOperations() {
      for(auto foo : allOperations) {
        printf("%c %c", foo.retroactiveOperation, foo.queueOperation);
    ...
    void writeOperationsOnFile(FILE *f) {
      ...
      for(auto foo : allOperations) {
        fprintf(f, "%c %c", foo.retroactiveOperation, foo.queueOperation);
...
int main(int argc, char *argv[]) {
  ...
  for(int i = start; i <= end; i += passo) {
    std::string str = "partialQueue" + std::to_string(i) + ".in";
    FILE *f = fopen(str.c_str(), "w");
    Tests::PartialQueueTest v(i);
