...
namespace Tests {
  ...
  class FullStackTest {
  public:
    struct FullStackTestOperation {
      char retroactiveOperation;
      char StackOperation;
      int t;
      int x;
      FullStackTestOperation(){};
      FullStackTestOperation(char _retroactiveOperation, char _StackOperation, int _t, int _x) {
    ...
    int q;
    std::vector< FullStackTestOperation > allOperations;
    FullStackTest(int _q) {
      q = _q;
      ...
      for(int i = 1; i <= q / 2; ++i) {
        ...
        used[t] = 1;
        x = Random::getInt(1, N);
        allOperations.push_back(FullStackTestOperation('I', 'E', t, x));
        tr.update(t, N, 1);
        ins.insert(t);
      }
      for(int i = q / 2 + 1; i <= q; ++i) {
        ...
        if(op == 0) {
          ...
          del.insert(t);
          tr.update(t, N, -1);
          allOperations.push_back(FullStackTestOperation('I', 'D', t, INT_MIN));
        }
        else if(op == 2) {
          int t;
          while(1) {
          ...
          ins.erase(t);
          tr.update(t, N, -1);
          allOperations.push_back(FullStackTestOperation('D', 'E', t, INT_MIN));
        }
        else if(op == 3) {
          int t = GetSample(del);
          del.erase(t);
          tr.update(t, N, 1);
          allOperations.push_back(FullStackTestOperation('D', 'D', t, INT_MIN));
        }
        else if(op == 4) {
          int t;
          while(1) {
          ...
          allOperations.push_back(FullStackTestOperation('Q', 'T', t, INT_MIN));
        }
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  for(int i = start; i <= end; i += passo) {
    std::string str = "FullStack" + std::to_string(i) + ".in";
    FILE *f = fopen(str.c_str(), "w");
    Tests::FullStackTest v(i);
